...
const Footer = () => (
    <footer className="bg-[#3B2A25] text-[#C9A27E] mt-20">
        <div className="max-w-7xl mx-auto px-6 py-14 grid grid-cols-1 md:grid-cols-4 gap-10">
            {/* Brand */}
            <div>
                <h3 className="text-2xl font-bold text-white mb-3">LoveBakes 🍰</h3>
                <p className="text-sm text-[#C9A27E] leading-relaxed">Homemade cakes, pastries and desserts crafted with love. Started with one imperfect cake for the girl he loved.</p>
            ...
            <div>
                ...
                <ul className="space-y-2 text-sm text-[#C9A27E]">
                    {['/', '/shop', '/custom-cake', '/about', '/contact'].map((path, i) => (
                        <li key={i}><Link to={path} className="hover:text-white transition-colors">{['Home', 'Shop', 'Custom Cake', 'About Us', 'Contact'][i]}</Link></li>
                    ))}
                </ul>
            </div>
            ...
            <div>
                ...
                <ul className="space-y-3 text-sm text-[#C9A27E]">
                    <li className="flex items-start gap-2"><MapPin className="w-4 h-4 mt-0.5 shrink-0" />Bhubaneswar, Odisha, India</li>
                    <li className="flex items-center gap-2"><Phone className="w-4 h-4 shrink-0" />+91 8144622958</li>
                    <li className="flex items-center gap-2"><Mail className="w-4 h-4 shrink-0" />sethysaiyangyadatta@gmail.com</li>
                </ul>
            </div>
        </div>
        <div className="border-t border-white/10 py-5 text-center text-xs text-[#C9A27E]">
