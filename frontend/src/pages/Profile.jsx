    ...
    return (
        <div className="max-w-3xl mx-auto px-4 py-12">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className="w-16 h-16 rounded-full bg-[#4A332C] flex items-center justify-center text-white text-2xl font-bold">
                    {user?.name?.charAt(0).toUpperCase()}
                </div>
                <div className="flex-1">
                    <h2 className="text-2xl font-bold text-[#4A332C]">{user?.name}</h2>
                    <p className="text-[#C8B6A6]">{user?.email}</p>
                    {user?.phone && <p className="text-[#C8B6A6] text-sm">{user.phone}</p>}
                </div>
            ...
            {/* Order History */}
            <h2 className="text-2xl font-bold text-[#4A332C] mb-5">Order History</h2>
            ...
            {loading ? (
                ...
                <div className="bg-white rounded-2xl p-12 text-center shadow-sm">
                    ...
                    <p className="text-[#4A332C] font-medium text-lg">No orders yet!</p>
                    <p className="text-[#C8B6A6] text-sm mt-1">Order your first cake today.</p>
                </div>
            ) : (
                <div className="space-y-4">
                    {orders.map(order => (
                        <motion.div key={order._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                            ...
                            <div className="flex justify-between items-start mb-3">
                                <div>
                                    <p className="text-xs text-[#C8B6A6] font-mono">#{order._id.slice(-8).toUpperCase()}</p>
                                    <p className="font-bold text-[#4A332C]">₹{order.totalPrice}</p>
                                </div>
                                <div className="flex items-center gap-1.5">
                                    {STATUS_ICON[order.orderStatus] || STATUS_ICON.pending}
                                    <span className="text-sm capitalize font-medium text-[#4A332C]">{order.orderStatus}</span>
                                </div>
                            </div>
                            <div className="text-sm text-[#4A332C]/70">
