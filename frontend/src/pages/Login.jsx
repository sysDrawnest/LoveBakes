...
const Login = () => {
    ...
    const inp = (hasError) => ({
        ...
        borderRadius: 12, fontSize: 13.5, color: '#3B2A25',
        fontFamily: "'DM Sans', sans-serif",
    ...
    const Field = ({ icon: Icon, label, error: fieldError, optional, children }) => (
        <div style={{ marginBottom: 14 }}>
            <label style={{
                ...
                textTransform: 'uppercase', color: '#A07060', marginBottom: 6,
                fontFamily: "'DM Sans', sans-serif",
            }}>
                {label}
            ...
            <div style={{ position: 'relative' }}>
                <Icon size={14} color={fieldError ? '#E85D75' : '#C9A27E'} style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none', zIndex: 1 }} />
                {children}
            </div>
            <AnimatePresence>
                {fieldError && (
                    <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                        style={{ color: '#E85D75', fontSize: 11, marginTop: 4, fontFamily: "'DM Sans', sans-serif" }}>
                        {fieldError}
                    </motion.p>
                )}
            </AnimatePresence>
            ...
            >
                ...
                <div style={{ textAlign: 'center', marginBottom: 22 }}>
                    <motion.div
                        ...
                        style={{ fontSize: 36, display: 'inline-block', marginBottom: 8 }}
                    >🎂</motion.div>
                    <h1 style={{
                        fontFamily: "'Playfair Display', serif",
                        fontSize: 26, fontWeight: 700, color: '#3B2A25',
                    ...
                    <AnimatePresence mode="wait">
                        <motion.p
                            ...
                            style={{ fontSize: 13, color: '#B08070', margin: 0, fontStyle: 'italic' }}
                        >
                ...
                <div style={{
                    ...
                    borderRadius: 50, padding: 3, marginBottom: 22, position: 'relative',
                }}>
                    {[['login', 'Login'], ['register', 'Sign Up']].map(([m, label]) => (
                        <button key={m} onClick={() => switchMode(m)} style={{
                ...
                <AnimatePresence>
                    {error && (
                        <motion.div
                            ...
                            style={{ background: '#FFF0EE', border: '1px solid #FCCACA', borderRadius: 10, padding: '10px 13px', fontSize: 12.5, color: '#C0392B' }}
                        >
                            ⚠️ {error}
                        </motion.div>
                    )}
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <Field icon={Mail} label="Email" error={errors.email && 'Valid email required'}>
                        <input {...register('email', { required: true, pattern: /^\S+@\S+\.\S+$/ })} type="email" placeholder="you@email.com" style={inp(errors.email)} />
                    </Field>
                    ...
                    <Field icon={Lock} label="Password" error={errors.password && 'Minimum 6 characters'}>
                        ...
                        <button type="button" onClick={() => setShowPassword(v => !v)} style={{
                            ...
                            background: 'none', border: 'none', cursor: 'pointer', padding: 3, zIndex: 1,
                        }}>
                            {showPassword ? <EyeOff size={14} color="#C9A27E" /> : <Eye size={14} color="#C9A27E" />}
                        </button>
                    </Field>

                    {mode === 'login' && (
                        <div style={{ textAlign: 'right', marginTop: -8, marginBottom: 16 }}>
                            <span style={{ fontSize: 11.5, color: '#D4705A', fontWeight: 600, cursor: 'pointer' }}>
                                Forgot password?
                            </span>
                        </div>
                    )}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '18px 0' }}>
                    ...
                    <span style={{ fontSize: 10.5, color: '#C9A27E', fontWeight: 600, letterSpacing: '0.12em' }}>OR</span>
                ...
                {/* Switch mode */}
                <p style={{ textAlign: 'center', marginTop: 18, marginBottom: 0, fontSize: 12.5, color: '#B08878' }}>
                    {mode === 'login' ? "Don't have an account? " : 'Already have an account? '}
                    <span
