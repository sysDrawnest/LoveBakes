    ...
    return (
        <div className="max-w-7xl mx-auto px-4 py-10">
            ...
            <div className="text-center mb-10">
                <h1 className="text-4xl font-bold text-[#4A332C]">Our Sweet Menu 🎂</h1>
                <p className="text-[#C8B6A6] mt-2">All items freshly baked to order</p>
            </div>

            {/* Search */}
            <div className="relative max-w-md mx-auto mb-8">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#C8B6A6]" />
                <input
                    value={search}
                    onChange={e => setSearch(e.target.value)}
                    placeholder="Search cakes, pastries..."
                    className="w-full pl-11 pr-5 py-3.5 bg-white border border-[#F1DEC9] rounded-full text-sm text-[#4A332C] focus:outline-none focus:ring-2 focus:ring-[#4A332C]/20"
                />
            </div>
                ...
                {CATEGORIES.map(c => (
                    <button
                        ...
                        className={`px-5 py-2.5 rounded-full text-sm font-semibold whitespace-nowrap transition-all ${category === c
                                ? 'bg-[#4A332C] text-white shadow-lg'
                                : 'bg-white border border-[#F1DEC9] text-[#4A332C] hover:border-[#4A332C]/30'
                            }`}
                    >
                        {c}
                    </button>
            ...
            {loading ? (
                ...
                <div className="text-center py-20 text-[#C8B6A6]">
